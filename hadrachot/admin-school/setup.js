...
async function loadTeachers() {
  ...
  // טעינה שנכשלה איננה "אין מורים" (14.9.26, עומס בשרת) — בלי זה הרשימה נראתה
  // ריקה והמנהלים התחילו להזין מחדש. השורות המקומיות והטיוטה לא נוגעות.
  if (!(res && res.ok)) {
    TS.toast('השרת עמוס — הרשימה השמורה עוד נטענת, לא צריך להזין מחדש. מנסים שוב…');
    setTimeout(loadTeachers, 5000);
    return;
  }
...
const SUBJECT_ALIASES = {
  ...
  'ספרויות': 'ספרות',
  'מורשת איסלאמית': 'מורשת אסלאמית', 'מורשת האסלאם': 'מורשת אסלאמית',
  'מורשת אסלאם': 'מורשת אסלאמית', 'תרבות אסלאמית': 'מורשת אסלאמית',
  'מורשת הדרוזים': 'מורשת דרוזית', 'מורשת דרוזים': 'מורשת דרוזית',
  'תרבות דרוזית': 'מורשת דרוזית'
};
