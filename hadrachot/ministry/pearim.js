...
// מקצועות שנלמדים רק בחלק מבתי הספר בתוך המגזר (מוסלמי מול דרוזי) — נספרים
// היכן שיש מורה, ולעולם לא נחשבים "פער" במקום שאין בו. בלי זה כל בית ספר
// ערבי היה מקבל שני פערים מדומים.
const ON_DEMAND = ['מורשת אסלאמית', 'מורשת דרוזית'];
...
function relevant(school, subject) {
  if (ON_DEMAND.includes(subject)) return !!(school.bySubject && school.bySubject[subject]);
