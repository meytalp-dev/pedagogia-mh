/* ============================================================
   ...
   ** טלפונים ומיילים אישיים נמצאים ב-assets/guides-contact.js **
   ...
   units / tracks — פיצול קבוצה בתוך אותו מקצוע (נוסף 9.9.26).
     units  — אילו רמות יח"ל שייכות למדריכ/ה: ['3'] · ['4-5'].
     tracks — אילו מסלולים: ['bagrut'] בלבד, או השמטה = בגרות וגמר.
     מדריכ/ה בלי units רואה את כל מורי המקצוע במגזרים שלה — בלי שינוי.
     מורה שטרם סומנה לו רמה מופיע אצל **שתי** המדריכות עם תווית "טרם סומן",
     כדי שאף אחד לא ייפול בין הכיסאות עד שהחלוקה תושלם.
     ↓ החלוקה במתמטיקה בחברה היהודית (קביעת מיטל 9.9.26):
       שירה = 3 יח"ל + כל הגמר · גל = 4-5 יח"ל · "3 + 4-5" מופיע אצל שתיהן.
...
window.TS_GUIDES = {
  ...
  nira: {
    /* המגזר החרדי ירד ממנה 10.9.26 — מורי הספרות החרדים עברו לשרה ברדה
       (קביעת מיטל). חלוקה נקייה: נירה = כללי, שרה = חרדי. */
    name: 'נירה הראל',
    subject: 'ספרות',
    sectors: ['kelali'],
    inspector: 'liat',
  ...
  sarah: {
    /* נוספה 10.9.26 — המדריכה החרדית הראשונה שנרשמת בנפרד מהמדריכה הכללית.
       sectors: ['haredi'] בלבד → הדשבורד שלה מציג רק מורי ספרות חרדים,
       ומורה שהיא מזינה נשמר אוטומטית עם sector=haredi.
       המפקח: יששכר חפץ, מפקח המגזר החרדי (קביעת מיטל) — ולא ליאת צבר
       שהיא מפקחת הספרות הארצית, ולכן שרה מופיעה במבט של יששכר. */
    name: 'שרה ברדה',
    subject: 'ספרות',
    sectors: ['haredi'],
    inspector: 'yisachar',
